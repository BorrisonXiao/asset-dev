...
def resolve_selection_metric(value):
    """Normalize the checkpoint-selection metric name to ``"WER"``/``"CER"``.

    WER is the LibriSpeech default; CER is the right key for languages whose
    transcripts carry no word spacing (zh/ja), where whitespace-split "WER"
    degenerates to a near-binary utterance error rate.
    """
    metric = str("wer" if value is None else value).upper()
    if metric not in ("WER", "CER"):
        raise ValueError(
            f"selection_metric must be 'wer' or 'cer', got {value!r}"
        )
    return metric
...
class SegmenterASR(ASR):
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if self.hparams.segmenter_mode == "coldstart":
            ...
            key_stat = resolve_selection_metric(
                getattr(self.hparams, "selection_metric", "wer")
            )
            key_kw = {"min_keys": [key_stat]}
...
def main(brain_class=SegmenterASR, configure_hparams=None):
    ...
    if hparams["segmenter_mode"] == "joint" and not hparams.get("skip_final_test", False):
        ...
        for k in test_datasets.keys():
            ...
            asr_brain.evaluate(
                test_datasets[k],
                min_key=resolve_selection_metric(
                    hparams.get("selection_metric", "wer")
                ),
